...
class Fazi extends Test {
    setup() {
        let fleet = new Fleet(1, [ShipTypes.getShipType(202, 1), ShipTypes.getShipType(203, 1)]);
        let player = new Player(1, [fleet]);
        this.attackers = new PlayerGroup([player]);

        fleet = new Fleet(2, [ShipTypes.getShipType(202, 1), ShipTypes.getShipType(203, 6546)]);
